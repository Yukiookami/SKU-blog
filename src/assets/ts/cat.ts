...
export default function (opts:any) {
  ...
  const $scroll_wrap:any = Opt.scrollWrapper || $content!.parentNode || document.body  // 内容元素的父元素
  ...
  try {
    $catalog!.innerHTML = `<div class='cl-wrapper'>${generateHtmlTree(tree, { id: -1 })}<svg class="cl-marker" width="200" height="200" xmlns="http://www.w3.org/2000/svg">
            <path stroke="#2c3e50" stroke-width="3" fill="transparent" stroke-dasharray="0, 0, 0, 1000" stroke-linecap="round" stroke-linejoin="round" transform="translate(-0.5, -0.5)" />
            </svg></div>`
  } catch (e) {
    console.error('error in progress-catalog', e)
  }
  
  const tocPath:any = $catalog!.querySelector('.cl-marker path')
  let tocItems:any, pathLength:any   // 左边svg-path长度
  ...
  function drawPath() {
    ...
    tocItems = tocItems.map(function(liDom:any) {
    ...
    tocItems = tocItems.filter((item:any) => !!item.target)
    
    const path:any = []
    let pathIndent:any
    
    tocItems.forEach(function(item:any, idx:any) {
  ...
  function coolScrollHandler() {
    ...
    let pathStart = pathLength,
      pathEnd = 0,
      visibleItems = 0
    tocItems.forEach(function(liItem:any) {
   ...
   * 点击事件
   */
  function clickHandler({ target }:any) {
  ...
  function getCatalogsTree(catalogs:any) {
    let title, tagName, tree = [], treeItem:any = {}, parentItem = { id: -1 }, lastTreeItem = null, id
   ...
   * @returns {*|Window}
   */
  function findParent(currTreeItem:any, lastTreeItem:any) {
   ...
   * @return {string}
   */
  function generateHtmlTree(tree:any, _parent:any) {
   ...
   * 判断是否是相同节点
   */
  function isEqual(node:any, node2:any) {
